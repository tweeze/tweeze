<?php
 ...
 if (!$db) echo "Beim Zugriff auf die Datenbank ist ein Fehler aufgetreten. Bitte versuchen Sie es sp&auml;ter nochmal.<br/><br/>";
  ...
  $query = "SELECT * FROM `$datenbank`.$wort";
  ...
  if(!$result) {
   ...
   while ($row=mysql_fetch_array($result,MYSQL_ASSOC)) {
   	...
   	$query1 = "SELECT count($id_dokument) FROM `$datenbank`.$dokument, `$datenbank`.$text WHERE $wort_id='$row[$id_wort]' AND $id_dokument=$dokument_id;";
    ...
    if(!$result1) {
     print "Fehler: " . mysql_error($connection);
   	...
   	$query3 = "UPDATE `$datenbank`.$wort SET idf='$IDF' WHERE $id_wort='$row[$id_wort]'";
